...
public class SpawnPredator : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (!gameOver) {
			...
			if (spawnedLastPredator) {
				if (GameObject.FindGameObjectsWithTag("Carnivore").Length == 0) {
					// Set winning bools to true
					drawYouWon = true;
					gameOver = true;

					// Trigger end of game
					GameObject.Find ("MainCamera").GetComponent<BuildMenu>().endGame();
				}
			}
		}
	}
	...
	*/
	void createCreature(int spawnLevel, int creatureId) {
		...
		// Return if creatureId <= 0
		if (creatureId <= 0)
			return;

		// Otherwise, spawn a predator
		// THOUGHT: I see both 'predator' and 'carnivore' used in variable names; should just stick to one to keep
		// things consistent.

		// Set the spawnPoints index as spawnLevel - 1, set currSpawnPoint accordingly (within row bounds)
		Vector3 currSpawnPoint = spawnPoints[spawnLevel > numRows ? 1 : spawnLevel - 1];

		// Instantiate the next predator; default to Lion creatureId is out of bounds (> carnivoreNames.Length)
		GameObject spawnedPredator = (GameObject) Instantiate
		(
			Resources.Load("Prefabs/Carnivore/" + carnivoreNames[creatureId > carnivoreNames.Length ? 1 : creatureId - 1]),
			currSpawnPoint,
			Quaternion.identity
		);
	}
