...
//					GUI.Label(new Rect(xPos - 42, yPos + 10, 80, 70), xAxisLabels[xMin + i], style);
		...
		// X-Axis Label
		style.alignment = TextAnchor.UpperCenter;
		// DH 2016-11-4  was hStart.y + 35
		GUI.Label (new Rect (hStart.x + (xAxisLength - 80) / 2, hStart.y + 25, 80, 30), "<color=yellow>" + xTitle + "</color>", style);
...
//		}
	...
	public void setOppGraph(bool oppGraph) {
		...
		if (oppGraph) {
			width = 540;   // was 900 with legend. DH 2016-11-4  was 690 before shrink
			height = 300;   // DH 2016-11-4 was 400 before shrink
			barWidth = 35;
			top = 100;
			left = (Screen.width - width) / 2;
			windowRect = new Rect (left, top, width, height);
			graphRect = new Rect (20, 30, 500, 225);
			redoAwake ();
		} else {
			width = 540;   // was 900 with legend. DH 2016-11-4  was 690 before shrink
			height = 300;   // DH 2016-11-4 was 400 before shrink
			...
			left = Screen.width - width - bufferBorder;
			windowRect = new Rect (left, top, width, height);
			graphRect = new Rect (20, 30, 500, 225);   // DH 2016-11-4 was 650, 325 before shrink
			redoAwake ();
		}
	}
	...
	void redoAwake() {
		...
		hStart = new Vector2 (85, graphRect.height - 50);    // DH 2016-11-4  75 -> 50
		hEnd = new Vector2 (graphRect.width - 50, hStart.y);
		...
		vStart = new Vector2 (hStart.x, hStart.y);
		vEnd = new Vector2 (vStart.x, 20);    // DH 2016-11-4  30 -> 20
