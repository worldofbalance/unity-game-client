...
public class ConnectionManager
{
    ...
    private int port;
    private NetworkProtocolTableBase networkProtocols;

    public ConnectionManager(NetworkProtocolTableBase npt)
    {
        networkProtocols = npt;
    }
    ...
    public List<NetworkResponse> Read(int numPackets = 20)
    {
        ...
        while (theStream.DataAvailable && responses.Count < numPackets)
        {
            ...
            Type pType = networkProtocols.Get(protocol_id);
            if (counter > 1)
            {
